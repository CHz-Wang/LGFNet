...
model.load_weights('/kaggle/input/parkinsons-freezing-submission-models/044_0.609_0.911_0.0481_model.h5') # TDCSFOG weights
...
model.load_weights('/kaggle/input/parkinsons-freezing-submission-models/062_0.601_0.913_0.0490_model.h5')
...
WEIGHTS = '/kaggle/input/parkinsons-freezing-submission-models/035_0.480_0.899_0.0480_model.h5' 
...
model = FOGModel()
model.build(input_shape=(GPU_BATCH_SIZE, CFG['block_size'] // CFG['patch_size'], CFG['patch_size']*3))
model.load_weights('/kaggle/input/parkinsons-freezing-submission-models/025_0.582_0.982_0.0463_model.h5')
...
model = FOGModel()
model.build(input_shape=(GPU_BATCH_SIZE, CFG['block_size'] // CFG['patch_size'], CFG['patch_size']*3))
model.load_weights('/kaggle/input/my-fog-dataset/099_0.604_0.918_0.0224_model.h5')
...
model.build(input_shape=(GPU_BATCH_SIZE, CFG['block_size'] // CFG['patch_size'], CFG['patch_size']*3))
model.load_weights('/kaggle/input/my-fog-dataset/115_0.644_0.911_0.0073_model.h5')
...
WEIGHTS = '/kaggle/input/parkinsons-freezing-submission-models/019_0.489_0.744_0.0817_model.h5'
...
for Id in defog_ids:
    ...
    submission = pd.DataFrame({'Id': (targets['Id'].values + '_' + targets['Time'].astype('str')).values,
                               'StartHesitation': targets['StartHesitation_prediction'].values*w,
                               'Turn': targets['Turn_prediction'].values*(w-0.1),
                               'Walking': targets['Walking_prediction'].values*w,
                              })
...
defog_path8 = [f"/kaggle/input/fog-ex185/ex185_{i}.pth" for i in range(5)]
...
for p in tqdm(defog_list):
    ...
    submission = pd.DataFrame({'Id': pred['Id'],
                               'StartHesitation': pred['StartHesitation'].values*w,
                               'Turn': pred['Turn'].values*(w+0.1),
                               'Walking': pred['Walking'].values*w,
                              })
